...
//     7: [["GE3791", 2], ["GE3751", 3], ["CCS361", 3], ["OMR351", 3], ["CRA332", 3], ["INTERN", 2]],
//     8: [["Project", 10]]
// };
...
function getTotalCreditsTillSemester(sem) {
    let sum = 0;
    for (let i = 1; i < sem; i++) {
        SUBJECTS_BY_SEMESTER[i].forEach(([_, credits]) => {
            sum += credits;
        });
    }
    return sum;
...
function calculateGPA() {
    ...
    if (semester === 1) {
        ...
        const previousCredits = getTotalCreditsTillSemester(semester);
        const totalWeightedPoints = (previousCGPA * previousCredits) + (GPA * totalCredits);
        const totalAllCredits = previousCredits + totalCredits;
        CGPA = totalWeightedPoints / totalAllCredits;
        console.log("CGPA :"+CGPA)
    }

    document.getElementById('resultBody').innerHTML = `<p>GPA: ${GPA.toFixed(2)}</p><p>CGPA: ${CGPA.toFixed(2)}</p>`;
    $('#resultModal').modal('show');
    document.getElementById("cgpaCalculatorModal").innerHTML = " ";
}
